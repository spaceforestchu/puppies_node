...
fetchPuppies = function(params){
  return new Promise(function(resolve, reject){
    Puppy.find(params, function(err, puppies){
      if(err){
        reject(err);
        return;
      }
...
createAccountLink = function(user) {
  var link = '';
...
router.get('/', function(req, res, next) {
  ...
  .then(function(user) {
    ...
    data['menu'] = {
      account: createAccountLink(user)
    }
    return fetchPuppies(null);
  })
...
//    res.render(page, data);
    return fetchPuppies({name: req.params.name})
  }).then(function(puppies){
    data['puppies'] = puppies;
    console.log(data);
    res.render(page, data);
